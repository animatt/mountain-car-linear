...
while converging:
    ...
    pos1, vel1 = np.random.uniform(
        low=-0.6, high=-0.4), 0  # position, velocity
    S1 = tiles.tiles(num_tilings, memsize,
                     (pos1 / tile_width, vel1 / tile_width))
    A1 = policy(S1, theta, epsilon)

    _, pos2, vel2 = bound(pos1, vel1 + .001 *
                          (A1 - 1) - .0025 * np.cos(3 * pos1))
    ...
    while game_in_progress:
        ...
        theta[A1][S1] += alpha * \
            (-1 + gamma * game_in_progress * q(S2, A2, theta) - q(S1, A1, theta))
